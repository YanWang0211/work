...
@interface ChatViewCell()<AVAudioPlayerDelegate>
...
//    _content.backgroundColor = RGBOF(0xd5d5d5);
    ...
    [_voiceBtn setImageEdgeInsets:UIEdgeInsetsMake(0, 45, 0, -45)];
    [_voiceBtn setTitleEdgeInsets:UIEdgeInsetsMake(0, -55, 0, 55)];
    
    _stateImage = [[UIImageView alloc] init];
    _stateImage.image = [UIImage imageNamed:@"chat_stateAni"];
    ...
    [_stateImage.layer addAnimation:rotationAnimation forKey:@"cellann"];
...
- (void)layoutSubviews{
    ...
    [_headImage mas_updateConstraints:^(MASConstraintMaker *make) {
        ...
        make.centerX.mas_equalTo(self.contentView.mas_right).offset(-30);
    }];
    
    [_userName  mas_updateConstraints:^(MASConstraintMaker *make) {
        make.width.mas_equalTo(kScreenW - 100);
        make.height.mas_equalTo(15);
        make.right.mas_equalTo(self.headImage.mas_left).offset(-10);
    ...
    [_content mas_updateConstraints:^(MASConstraintMaker *make) {
        ...
        make.right.mas_equalTo(self.userName.mas_right).offset(- 10);
    ...
    [_stateImage mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.mas_equalTo(15);
        make.right.mas_equalTo(self.backImage.mas_left).offset(-5);
    ...
    if (_chatModel.status == kJMSGMessageStatusSendDraft) {
        _stateImage.hidden = NO;
    }else if(_chatModel.status == kJMSGMessageStatusSendSucceed){
...
- (void)handleTap{
    [self.delegate headImageClickEvent:self.chatModel];
}
