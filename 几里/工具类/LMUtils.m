...
+ (UIImageView *)dottedLine:(CGRect)frame
{
    ...
    CGContextSetStrokeColorWithColor(line, RGBA(200, 200, 200, 1).CGColor);
...
{
    return [LMUtils imageWithColor:color andSize:CGSizeMake(1.0f, 1.0f)];
}
...
{
    return [LMUtils resizableImage:imageName edgeInsets:UIEdgeInsetsMake(0.5, 0.5, 0.5, 0.5)];
}
...
+ (BOOL)isBlankString:(NSString *)string
{
    ...
    if ([string isEqualToString:@"(null)"] || [string isEqualToString:@"<nill>"])
    {
        return YES;
    }
    return NO;
...
+ (NSInteger) getNowTimestamp{
    ...
    NSInteger timesp = [[NSNumber numberWithDouble:[localeDate timeIntervalSince1970]] integerValue];
    return timesp;
}
...
{
    if ([LMUtils isBlankString:jsonStr])
    {
        return nil;
    }
